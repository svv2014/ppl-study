# 2026 Mobile-First Design Research — PPL Study
...
**Recommendation for #60:** Add a bottom `BottomNavigation` bar on xs–sm breakpoints with ≤ 5
items; keep the desktop AppBar.
...
primary buttons. Diluting the signal reduces its impact.

**Recommendation for #61:** Treat `#f5a623` as a signal-only colour; introduce a `border-subtle`
token for outlines and a muted `accent-muted` for non-CTA highlights.
...
(base) and `#0d1f3c` (paper) but nothing for elevated modals/drawers.

**Recommendation for #61:** Add `background.surfaceRaised: '#112244'` as a third tier.
...
motivation signal disappears.

**Recommendation for #60 (nav):** Add a small `completedCount / total` chip to the nav bar so
progress is always visible without requiring a trip to Home.
...
flag plus TypeScript module augmentations for custom tokens.

**Recommendation for #61.**

---

## Palette Direction: Keep Aviation DNA, Refine Token Depth
...
- Implementing any code changes (that is each child issue's job)
- Native app or PWA work
- Light mode (CSS variables in #61 lay the groundwork; actual light theme is a future ticket)
- New npm package decisions (noted as optional in typography section — decide in #59)
- Firestore, auth, or backend work
